...
async def github_oauth_redirect(state: Optional[str] = None):
    """
    Redirect user to GitHub OAuth authorization page

    Args:
        state: CSRF protection token (generated by frontend, validated on callback)
    ...
    # Pass state for CSRF protection (frontend validates on callback)
    if state:
        github_auth_url += f"&state={state}"

    logging.info(f"🔐 Redirecting to GitHub OAuth (state={'present' if state else 'none'})")
